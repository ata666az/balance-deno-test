// ====================================================================
// DENO DEPLOY - ULTRA OPTIMIZED PROXY
// Race, Parallel, Failover, Abort, Timeout 1s per batch
// Fixed: body handling via ArrayBuffer, header sanitization
// ====================================================================

const WORKER_URLS: string[] = [
...
// ==== Konfigurasi ====
const BATCH_SIZE = 4;              // jumlah backend per batch
const BATCH_TIMEOUT_MS = 1000;     // timeout per batch (ms)
const MAX_BODY_BYTES = 1024 * 1024; // batas body 1MB untuk keamanan memori
...
async function handler(request: Request): Promise<Response> {
  try {
    ...
    // Deteksi apakah request punya body
    const hasBody = request.method !== 'GET' && request.method !== 'HEAD';

    // Baca body menjadi ArrayBuffer SEKALI
    ...
    // Sanitasi header: buang header yang bermasalah saat di-forward
    ...
    const shuffled = shuffle(WORKER_URLS);
    ...
    for (let i = 0; i < shuffled.length; i += BATCH_SIZE) {
      ...
      const promises = batch.map(async (host): Promise<Response> => {
        ...
        try {
          ...
          // Tolak 429 (rate limit) dan 5xx (server error)
...
// ==== Jalankan server ====
Deno.serve(handler);  'cf.janji5.workers.dev',
  'cf.janji6.workers.dev',
  'cf.janji7.workers.dev',
  'cf.janji8.workers.dev',
  'cf.janji9.workers.dev',
