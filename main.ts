// ====================================================================
// DENO DEPLOY - ULTRA OPTIMIZED PROXY (FIXED VERSION)
// Race, Parallel, Failover, Timeout 1s per batch
// Fix: Promise.race instead of AbortController (avoid Deno fetch bug)
// ====================================================================
...
// Timeout wrapper untuk satu promise
...
async function handler(request) {
  try {
    ...
    for (var i = 0; i < shuffled.length; i += BATCH_SIZE) {
      ...
      // Buat promise untuk tiap host di batch, tanpa AbortController
      ...
      try {
        // Race antara semua request di batch dengan timeout
        ...
        // Batch ini gagal atau timeout, lanjut ke batch berikutnya
        continue;
      }
    }
