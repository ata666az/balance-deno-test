// ====================================================================
// DENO DEPLOY - ULTRA OPTIMIZED PROXY (WebSocket Support)
// Support: HTTP forward + WebSocket bridge
// ====================================================================
...
var BATCH_SIZE = 4;
var BATCH_TIMEOUT_MS = 1000;
var WS_BATCH_SIZE = 4;
var WS_TIMEOUT_MS = 2500;
...
// HANDLER WEBSOCKET
// ====================================================================
async function handleWebSocket(request, pathAndQuery) {
  var shuffled = shuffle(WORKER_URLS);

  // Coba batch demi batch sampai ketemu backend WS yang hidup
  for (var i = 0; i < shuffled.length; i += WS_BATCH_SIZE) {
    var batch = shuffled.slice(i, i + WS_BATCH_SIZE);

    // Coba connect WS ke semua backend di batch secara paralel
    var attempts = batch.map(function (host) {
      return new Promise(function (resolve) {
        var settled = false;
        var ws;
        try {
          ws = new WebSocket('wss://' + host + pathAndQuery);
        } catch (e) {
          return resolve(null);
        }
    ...
    // Tutup WS lain yang berhasil (tidak terpakai)
    for (var m = 0; m < results.length; m++) {
      if (results[m] && results[m] !== winner) {
        try { results[m].close(); } catch (e) {}
      }
    }

    if (!winner) continue;

    // Upgrade koneksi client
    var upgrade;
    try {
      upgrade = Deno.upgradeWebSocket(request);
    } catch (e) {
      try { winner.close(); } catch (err) {}
      return new Response('WS upgrade failed: ' + (e && e.message ? e.message : String(e)), {
        status: 500
      });
    }

    var clientSocket = upgrade.socket;
    var backendWs = winner;
    var closed = false;
    ...
    clientSocket.onopen = function () {
      // Client siap; backend sudah OPEN
    };

    clientSocket.onmessage = function (event) {
      if (backendWs.readyState === WebSocket.OPEN) {
        try { backendWs.send(event.data); } catch (e) { closeBoth(); }
      }
    };

    clientSocket.onclose = function () { closeBoth(); };
    clientSocket.onerror = function () { closeBoth(); };

    backendWs.onmessage = function (event) {
      if (clientSocket.readyState === WebSocket.OPEN) {
        try { clientSocket.send(event.data); } catch (e) { closeBoth(); }
      }
    };

    backendWs.onclose = function () { closeBoth(); };
    backendWs.onerror = function () { closeBoth(); };

    return upgrade.response;
  }

  return new Response('No backend available for WebSocket', {
    status: 503,
...
// HANDLER HTTP BIASA
// ====================================================================
async function handleHttp(request, pathAndQuery, bodyBuffer, forwardHeaders) {
  var shuffled = shuffle(WORKER_URLS);

  for (var i = 0; i < shuffled.length; i += BATCH_SIZE) {
    var batch = shuffled.slice(i, i + BATCH_SIZE);

    var promises = batch.map(function (host) {
      return (async function () {
        var targetUrl = 'https://' + host + pathAndQuery;
    ...
    try {
      var fastest = await Promise.race([
        ...
        new Promise(function (_, reject) {
          setTimeout(function () { reject(new Error('Batch timeout')); }, BATCH_TIMEOUT_MS);
        })
      ]);
      return fastest;
    } catch (e) {
...
async function handler(request) {
  try {
    ...
    if (upgradeHeader && upgradeHeader.toLowerCase() === 'websocket') {
      return await handleWebSocket(request, pathAndQuery);
    }
    ...
    return await handleHttp(request, pathAndQuery, bodyBuffer, forwardHeaders);
  } catch (err) {
